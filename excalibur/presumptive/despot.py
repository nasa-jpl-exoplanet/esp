...
def _url(arg:str):
    try:
        ...
        if all((result.scheme, result.netloc)):
            ...
            raise ArgumentTypeError(f"Invalid URL: '{arg}' does not have a scheme or network location.")
    except ValueError:
        raise ArgumentTypeError(f"Invalid URL format: '{arg}'")
...
def tyrannize():
    ...
    wap.add_argument('-n', '--nodes',
                     nargs='+',
                     required=True,
                     type=int,
                     help='mentor node numbers')
    wap.add_argument('-r', '--replicas',
                     required=True,
                     type=int,
