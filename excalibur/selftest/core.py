    ...
    for targetlist in analysistargetlists:
        ...
        for trgt in targetlist['targets']:
            ...
            if trgt not in aspecttargets:
                log.warning(
                    '--< SELFTEST ANALYSIS: TARGET NOT IN ASPECT %s %s >--',
                    filt,
                    trgt,
                )
                ...
                log.warning(
                    '--< SELFTEST ANALYSIS: FORMAT ERROR - NO STATUS %s %s >--',
                    filt,
                    trgt,
                )
            else:
                ...
                if 'stellar_params' in atmos_fit['data'].keys():
                    ...
                    stellar_feh = 0
                    log.warning(
                        '--< SELFTEST ANALYSIS: no FEH* for %s >--', trgt
                    )

                # verify SV succeeded for target
                if not atmos_fit['STATUS'][-1]:
                    log.warning(
                        '--< SELFTEST ANALYSIS: STATUS IS FALSE FOR CERB.ATMOS %s %s >--',
                        filt,
                        trgt,
                    )
                else:
