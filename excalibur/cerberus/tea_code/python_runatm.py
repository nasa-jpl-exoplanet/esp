...
def _multiproc_worker(pres_arr, temp_arr, atom_arr,
                      free_energy, heat, stoich_arr, guess,
                      maxiter, verb, times, xtol,
                      start, end, shared_abn):
    
    """Exactly the same per-layer loop as original runatm, minus file I/O."""
    abn = np.ctypeslib.as_array(shared_abn.get_obj()).reshape((len(pres_arr),
                                                               len(stoich_arr)))
    for q in range(start, end):
        ...
        try:
            ...
            for n in range(max_retry):
                result = it.iterate(
                    pres_arr[q], stoich_arr, atom_arr[q], g_RT,
                    maxiter, verb, times, guess, xtol)
...
def run_tea(pre_atm, cfg_file, desc="tea_output"):
    ...
    missing = [sp for sp in speclist
               if not any((thermo_dir / f"{sp}{ext}").exists() for ext in (".dat", ".txt"))]
    if missing:
    ...
    for n in range(ncpu):
        ...
        p = mp.Process(target=_multiproc_worker,
                       args=(pres_arr, temp_arr, atom_arr,
                             free_energy, heat, stoich_arr, guess,
                             maxiter, verb, times, xtol,
                             s, e, shared_abn))
        procs.append(p)
