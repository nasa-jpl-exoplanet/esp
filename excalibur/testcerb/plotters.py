...
def plot_fits_vs_truth(
    ...
    for param in paramlist:
        ...
        for testIndex, (truth, fit, error, error2) in enumerate(zip(
                truth_values[param], fit_values[param], fit_errors[param], fit_errors2sigma[param])
        ):
            ...
            ax.errorbar(
                ...
                # yerr=error,
                fmt='.', color='k', lw=2, zorder=4
            )
            ax.errorbar(
                testIndex + 1, fit,
                yerr=np.array([[error2[0]], [error2[1]]]),
                fmt='.', color='grey', lw=0.5, zorder=4
            )
        ...
        if param == 'T':
            ...
            plt.title(
                'C/O retrieval for ' + str(Niter) + ' planets'
            )
        else:
            plt.title(
                param
        ...
        for truth, traces in zip(truth_values[param], all_traces):  # loop over each target
        ...
        lower = prior_ranges[param][0]
        upper = prior_ranges[param][1]
        ...
        ylim = ax3.get_ylim()
        plt.plot([truth_values[param][0],truth_values[param][0]], [0,10], 
                 c='k', ls=':',lw=1)
        ax3.set_ylim(ylim)
