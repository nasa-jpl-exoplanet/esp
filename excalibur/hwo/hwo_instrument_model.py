...
def load_hwo_instrument(target):
    ...
    # star properties
    BB = 2 * const.h * const.c**2 / wavelength**5 / (np.exp(const.h * const.c / (wavelength * const.k * Tstar)) - 1.0) # flux per wl per solid angle (blackbody spectral radiance)
    ...
    photons_in_transit = photon_rate * transit_in # total photons in each bin during the transit
    photons_out_transit = photon_rate * transit_out # total photons in each bin out of transit

    # noise
    shotnoise = np.sqrt(1/photons_in_transit + 1/photons_out_transit) # shot noise per bin
