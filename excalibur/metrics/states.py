...
class CpuAndMem(dawgie.StateVector):
    ...
    def __init__(self, name, mds):
        ...
        for md in mds:
            if md.run_id not in self['byrid']: sel['byrid'][md.run_id] = []
            self['byrid'][md.run_id].append(md.sv)
            if md.target not in self['bytn']: self['bytn'][md.target] = []
    ...
    def view(self, caller: excalibur.Identity, visitor: dawgie.Visitor) -> None:
        ...
        c1r2 = plot_resource(self['byrid'], sorted(self['byrid'].keys()),
                             self._name, 'Run ID', False)
        c1r2.x_range = c1r1.x_range
        c2r1 = plot_resource(self['bytn'], sorted(self['bytn'].keys()),
                             self._name, 'Target Name', True)
        c2r2 = plot_resource(self['bytn'], sorted(self['bytn'].keys()),
                             self._name, 'Target Name', False)
        c2r2.x_range = c2r1.x_range
        c2r1.y_range = c1r1.y_range
        c2r2.y_range = c1r2.y_range
        f = bokeh.layouts.gridplot([[c1r1, c2r1],[c1r2, c2r2]],
...
def plot_resource (data_table, keys, tan, xlabel, mem):
    ...
    yx = []
    for svl in svs:
        w = [(sv['db_memory' if mem else 'task_wall'].value() + 
              sv['task_memory' if mem else 'task_wall'].value())*(1024 if mem else 1)
             for sv in svl]
    ...
    fig.xaxis.formatter = bokeh.models.CustomJSTickFormatter(code=f'''
var labels = {keys};
return labels[tick];''')
    fig.xaxis.major_label_orientation = math.pi/4
    fig.yaxis.axis_label = 'Memory [B]' if mem else 'Wall Clock [days H:M:S]'
    if mem:
        fig.yaxis.formatter = bokeh.models.CustomJSTickFormatter(code='''
